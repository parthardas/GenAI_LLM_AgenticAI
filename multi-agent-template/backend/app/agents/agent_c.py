...
def tool_one(tool_input: InputSchema) -> OutputSchema:
    ...
    """
    logger.info("Hi! I am agent C Tool 1")
    return OutputSchema(result=f"Handled by Agent C tool_one: {tool_input.data}")
...
def tool_two(tool_input: InputSchema) -> OutputSchema:
    ...
    Returns:
        OutputSchema: An object containing the processed result.
    """
    logger.info("Hi! I am agent C Tool 2")
    return OutputSchema(result=f"Handled by Agent C tool_two: {tool_input.data}")
...
def agent_c_node(state:GraphState) -> GraphState:
    """
    Processes the given state for agent C, determines which tool to invoke based on user input,
    ...
    if "tool 1" in user_input.lower():
        ...
        response = "Please specify 'tool 1' or 'tool 2' in your input for agent C."
